"""Fine-attention-only latency: stock fa4_hybrid path vs fa4_hybrid_v2
(ping-pong), interleaved in one invocation to cancel clock drift.

Exact VSA bench fine-stage shapes (per-GPU after ulysses 8): B=1, D=128,
Hq=8, Hkv=1, num_blocks cubes of 64 tokens, topk selected per query cube.
FLOPs = 4 * 64^2 * D * num_blocks * topk * heads (identical for both).

Usage: python bench_fine_v2.py [sizes] [kernels]
  sizes:   comma list from {16k,32k,64k,128k} (default all)
  kernels: comma list from {stock,v2} (default both)
"""
...
def time_kernel(q, k, v, bs, pp_env, warmup=10, rep=50):
    os.environ["FLASH_ATTN_SM90_PP"] = pp_env
...
def main():
    ...
    kernels = sys.argv[2].split(",") if len(sys.argv) > 2 else list(KERNELS)
    print(f"{'size':>6} {'kernel':>8} {'ms':>9} {'TFLOP/s':>9} {'MFU':>7}")
    for name in sizes:
        nkv, topk = SIZES[name]
        q, k, v, bs = make_inputs(nkv, topk)
        flops = 4 * BLK * BLK * 128 * nkv * topk * 8
        for label in kernels:
            ms = time_kernel(q, k, v, bs, KERNELS[label])
