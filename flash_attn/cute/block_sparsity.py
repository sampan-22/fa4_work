...
def normalize_block_sparse_config(
    ...
    *,
    ...
    """Validate and normalize a block-sparse config.

    When allow_kv_pairing is set (SM90 forward), tile_n may be exactly
    2 * sparse_block_size[1]: the kernel then packs two independently
    selected sparse KV blocks into each tile_n-wide MMA tile
    (kv_pair_factor == 2). The sparse index/count tensors keep their
    sparse-block-granularity shapes. Pairing supports only the full-block
    list; callers must pass mask_block_cnt == 0 everywhere.
    """
    ...
    if (
        ...
        if kv_pair_factor != 2:
            raise ValueError(
                "KV pairing requires tile_n == 2 * sparse_block_size[1]; got "
                f"tile_n={n_block_size}, sparse_block_size[1]={sparse_block_size_kv}."
            )
    ...
    expected_count_shape, expected_index_shape, q_subtile_factor = (
        infer_block_sparse_expected_shapes(
            ...
            # Expected n-block shapes are in sparse-block units.
