"""Fine-attention-only latency: stock (FLASH_ATTN_SM90_FIXED_REF_SOFTMAX=0)
vs frozen-ref (FLASH_ATTN_SM90_FIXED_REF_SOFTMAX=1) on the SM90 forward,
block-sparse VSA shapes. Exact VSA bench fine-stage shapes (per-GPU after
ulysses 8): B=1, D=128, Hq=8, Hkv=1, num_blocks cubes of 64 tokens, topk
selected per query cube. FLOPs = 4 * 64^2 * D * num_blocks * topk * heads.

Usage: python bench_fixed_ref_softmax.py [sizes]
...
def time_kernel(q, k, v, bs, fixed_ref, warmup=10, rep=50):
...
def main():
    ...
    print(f"{'size':>6} {'trial':>5} {'cfg':>8} {'ms':>9} {'TFLOP/s':>9} {'MFU':>7}")
    for name in sizes:
        nkv, topk = SIZES[name]
        q, k, v, bs = make_inputs(nkv, topk)
        flops = 4 * BLK * BLK * 128 * nkv * topk * 8
        for trial in range(1, trials + 1):
            for label, fixed_ref in [("stock", False), ("frozen", True)]:
                ms = time_kernel(q, k, v, bs, fixed_ref)
                tf = flops / (ms * 1e-3) / 1e12
                print(
                    f"{name:>6} {trial:5d} {label:>8} {ms:9.3f} {tf:9.1f} "
