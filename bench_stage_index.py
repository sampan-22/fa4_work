"""Fine-attention-only latency: baseline (GMEM index reads) vs staged
(FLASH_ATTN_SM90_STAGE_INDEX=1, index list cooperatively staged into SMEM
once per tile before warp specialization) on the SM90 forward, block-sparse
VSA shapes. Exact VSA bench fine-stage shapes (per-GPU after ulysses 8):
B=1, D=128, Hq=8, Hkv=1, num_blocks cubes of 64 tokens, topk selected per
query cube. FLOPs = 4 * 64^2 * D * num_blocks * topk * heads.

Usage: python bench_stage_index.py [sizes]
...
def time_kernel(q, k, v, bs, stage_index, warmup=10, rep=50):
    os.environ["FLASH_ATTN_SM90_STAGE_INDEX"] = str(stage_index)
...
def main():
    ...
    for name in sizes:
        ...
        for label, stage_index in [("base", 0), ("staged", 1)]:
            ms = time_kernel(q, k, v, bs, stage_index)
